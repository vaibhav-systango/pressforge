...
export async function GET(
  ...
  // Forward the OAuth callback to backend FastAPI server
  ...
  try {
    ...
    if (locationHeader) {
      ...
      // If backend redirects to localhost:3000 or 127.0.0.1:3000, rewrite to current request origin (e.g. ngrok domain)
      if (
